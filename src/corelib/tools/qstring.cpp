...
QString& QString::replace(QChar ch, const QString &after, Qt::CaseSensitivity cs)
{
    ...
    if (after.size() == 1)
        return replace(ch, after.constData()[0], cs);
...
bool operator==(const QString &s1, const QString &s2) Q_DECL_NOTHROW
{
    ...
    return qMemEquals(s1.d.b, s2.d.b, s1.d.size);
}
    ...
    */
    while (index != -1) {
        ...
        while (i < pos) {
            ...
            memcpy(uc, d.b + copystart, size * sizeof(QChar));
            uc += size;
            memcpy(uc, after.d.b, al * sizeof(QChar));
            uc += al;
            copystart = copyend + replacements[i].length;
            i++;
        }
        memcpy(uc, d.b + copystart, (d.size - copystart) * sizeof(QChar));
...
bool QString::startsWith(QChar c, Qt::CaseSensitivity cs) const
{
    return d.size
           && (cs == Qt::CaseSensitive
               ? d.b[0] == c
               : foldCase(d.b[0]) == foldCase(c.unicode()));
}
...
bool QString::endsWith(QChar c, Qt::CaseSensitivity cs) const
{
    return d.size
           && (cs == Qt::CaseSensitive
               ? d.b[d.size - 1] == c
               : foldCase(d.b[d.size - 1]) == foldCase(c.unicode()));
}
...
QByteArray QString::toLatin1_helper_inplace(QString &s)
{
    ...
    ba_d.d->asAllocatedData()->alloc *= sizeof(ushort);
...
QVector<uint> QString::toUcs4() const
{
    ...
    uint *a = v.data();
    int len = toUcs4_helper(d.b, length(), a);
    v.resize(len);
    return v;
}
...
int QString::compare(const QString &other, Qt::CaseSensitivity cs) const Q_DECL_NOTHROW
{
    ...
    return ucstricmp(d.b, d.b + d.size, other.d.b, other.d.b + other.d.size);
}
...
QString QString::rightJustified(int width, QChar fill, bool truncate) const
{
    ...
    if (padlen > 0) {
        ...
        while (padlen--)
           * uc++ = fill;
        if (len)
            memcpy(uc, d.b, sizeof(QChar)*len);
    } else {
        if (truncate)
            result = left(width);
        else
