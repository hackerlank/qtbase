...
void QFileSystemMetaData::fillFromStatBuf(const QT_STATBUF &statBuffer)
{
    ...
    if (!metadataChangeTime_)
        metadataChangeTime_ = modificationTime_;
    accessTime_ = timespecToMSecs(statBuffer.st_atim);
#else
    metadataChangeTime_ = qint64(statBuffer.st_ctime ? statBuffer.st_ctime : statBuffer.st_mtime) * 1000;
