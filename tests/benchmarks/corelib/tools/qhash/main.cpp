...
private slots:
    ...
    void hashing_crc32String_data() { data(); }
    void hashing_crc32String() { hashing_template<Crc32String>(); }
    void hashing_sipHashString_data() { data(); }
    void hashing_sipHashString() { hashing_template<SipHashString>(); }
    void hashing_allansSipHashString_data() { data(); }
    void hashing_allansSipHashString() { hashing_template<AllansSipHashString>(); }
    void hashing_aesHashString_data() { data(); }
    void hashing_aesHashString() { hashing_template<AesHashString>(); }
...
void tst_QHash::initTestCase()
{
    ...
    QFile *smallPathsData = new QFile(QFINDTESTDATA("hash.dump"));
    QVERIFY(smallPathsData->open(QIODevice::ReadOnly));
    uchar *const begin = smallPathsData->map(0, smallPathsData->size());
    uchar *const end = begin + smallPathsData->size();
    qint64 totalbytes = 0;
    for (auto ptr = begin; ptr != end; ) {
        int len;
        Q_ASSERT(ptr + sizeof(len) < end);
        memcpy(&len, ptr, sizeof(len));
        ptr += 4;
        Q_ASSERT(ptr + len <= end);

        bool even = ((len % 2) == 0);

        QString s = even ? QString::fromRawData(reinterpret_cast<QChar *>(ptr), len / 2) :
                           QString(len / 2, Qt::Uninitialized);
        if (!even) {
            memcpy(s.data(), ptr, len);
            reinterpret_cast<uchar *>(s.data())[len] = '\0';
        }
        ptr += len;
        totalbytes += sizeof(QChar) * s.size();
        smallFilePaths << s;
    }
    qDebug() << "Loaded" << smallFilePaths.size() << "strings, totalling" << totalbytes << "bytes";
